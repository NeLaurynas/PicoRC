...
void lights_init() {
	...
	const auto all_div = utils_calculate_pwm_divider(cfg_all_pwm_top, 1);
	...
	gpio_set_drive_strength(MOD_LIGHTS_TAIL, GPIO_DRIVE_STRENGTH_8MA);
...
void lights_tail() {
	if (!current_state.lights.tail && !current_state.lights.braking) pwm_set_chan_level(cfg_tail_slice, cfg_tail_channel, 0);
	else {
		if (current_state.lights.braking) pwm_set_chan_level(cfg_tail_slice, cfg_tail_channel, 100);
		else pwm_set_chan_level(cfg_tail_slice, cfg_tail_channel, 10);
	}
}
