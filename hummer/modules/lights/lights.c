...
static i32 state_blinkers_frame = 0; // why not in global state?
...
void lights_init() {
	...
	// init BEAMS
	gpio_init(MOD_LIGHTS_BEAM);
	gpio_set_dir(MOD_LIGHTS_BEAM, true);
...
void lights_head_up() {
	switch (current_state.lights.head) {
		case 0:
			...
			gpio_put(MOD_LIGHTS_BEAM, false);
			break;
		case 1:
			pwm_set_chan_level(cfg_head_slice, cfg_head_channel, 15);
			gpio_put(MOD_LIGHTS_BEAM, false);
			break;
		case 2:
			pwm_set_chan_level(cfg_head_slice, cfg_head_channel, 100);
			gpio_put(MOD_LIGHTS_BEAM, true);
			break;
		default:
...
static void blinkers_anim() {
	...
	if (state_blinkers_frame < tim_1) {
		...
		if (current_state.lights.blinkers_left || current_state.lights.hazards) pwm_set_chan_level(cfg_blinkers_slice_left, cfg_blinkers_channel_left, level);
		if (current_state.lights.blinkers_right || current_state.lights.hazards) pwm_set_chan_level(cfg_blinkers_slice_right, cfg_blinkers_channel_right, level);
	} else {
...
void lights_animation() {
	if (current_state.lights.blinkers_left || current_state.lights.blinkers_right || current_state.lights.hazards) blinkers_anim();
}
